...
export const sanearHTML = (texto) => {
    const aux = document.createElement("div");
    aux.textContent = texto ?? "";
    return aux.innerHTML;
};
export const crearTarjetaHtml = ({ id, nombre, precio, stock, imagen, marca, valoracion }) => `
...
`;
export const crearFilaCarritoHtml = ({ id, nombre, precio, cantidad }) => `
...
`;
export const crearDetalleHtml = (prod) => `
  <div class="relative">
...
`;
export const crearAvisoHtml = (mensaje) => `
  <p class="col-span-full text-center text-texto-suave py-8">${sanearHTML(mensaje)}</p>
`;
export const crearAcordeonPedidoHtml = (p, index) => `
