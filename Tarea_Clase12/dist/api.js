...
const mapearCategoria = (cat) => cat === "mobile-accessories" ? "audio" : cat;
const estructurarProducto = (item, catOriginal = item.category) => ({
    id: item.id,
    nombre: item.title,
    precio: item.price,
...
async function pedirPorCategoria(cat) {
    const res = await fetch(`${API_BASE}/products/category/${cat}`);
    if (!res.ok)
        throw new Error(`Status Code: ${res.status}`);
    const data = await res.json();
    return data.products.map((p) => estructurarProducto(p, cat));
}
export async function obtenerProductosTechCart() {
    try {
...
export async function obtenerDetalleProducto(id) {
    ...
    if (!res.ok)
        throw new Error("Error consultando el producto.");
